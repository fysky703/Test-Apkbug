...
import '../services/quick_controls_service.dart';
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  final QuickControlsService _quickControls = QuickControlsService();
  ...
  Future<void> _quickAction(String title, Future<String> Function() action) async {
    try {
      final result = await action();
      if (!mounted) return;

      String message = result;
      if (result == 'DEVICE_ADMIN_REQUIRED') {
        await _quickControls.requestDeviceAdmin();
        message = 'Enable Device Admin, then press Lock again.';
      } else if (result == 'CAMERA_PERMISSION_REQUIRED') {
        await _quickControls.requestCameraPermission();
        message = 'Camera permission requested.';
      } else if (result == 'FLASH_ON') {
        message = 'Flashlight ON';
      } else if (result == 'FLASH_OFF') {
        message = 'Flashlight OFF';
      } else if (result == 'LOCKED') {
        message = 'Screen locked.';
      } else if (result == 'WAKE_REQUESTED') {
        message = 'Screen wake requested.';
      } else if (result == 'ALERT_PLAYED') {
        message = 'Alert sound played.';
      } else if (result == 'VIBRATED') {
  ...
  Widget _buildQuickControls(bool isDark) {
    final surface = isDark ? const Color(0xFF151D30) : Colors.white;
    final border = isDark
      ...
      return Expanded(
        child: Padding(
          ...
          child: Material(
            ...
            child: InkWell(
              borderRadius: BorderRadius.circular(16),
              onTap: () => _quickAction(label, onTap),
              child: Container(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: surface.withOpacity(isDark ? 0.92 : 0.98),
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 5),
          Text(
            'Buttons call Android APIs directly.',
          ...
          Row(
            children: [
              button(
                ...
                label: 'Lock',
                onTap: _quickControls.lockScreen,
              ),
              button(
                icon: Icons.wb_sunny_rounded,
                label: 'Wake',
                onTap: _quickControls.wakeScreen,
              ),
              button(
                icon: Icons.flashlight_on_rounded,
                label: 'Flash ON',
                onTap: () => _quickControls.setFlash(true),
              ),
              button(
                icon: Icons.flashlight_off_rounded,
                label: 'Flash OFF',
                onTap: () => _quickControls.setFlash(false),
              ),
            ],
          ),
          Row(
            children: [
              button(
                ...
                label: 'Alert',
                onTap: _quickControls.playAlert,
              ),
              button(
                icon: Icons.vibration_rounded,
                label: 'Vibrate',
                onTap: _quickControls.vibrate,
              ),
              button(
                icon: Icons.volume_down_rounded,
                label: 'Volume 100%',
                onTap: () async {
                  return _actionHandler.setVolumeDirect(100);
                },
              ),
              button(
        ...
        children: [
          ...
          Column(
            children: [
              ...
              // Direct hardware/device controls. These do not use AI or UI scrolling.
